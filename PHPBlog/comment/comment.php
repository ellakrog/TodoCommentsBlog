...
class Comment{
    ...
    public function CommentUsersBlog(){

        $query = "SELECT  comment.content, comment.id_blog,comment.id_user,comment.created, blogs.id_blog,users.name
            FROM comment
            
            INNER JOIN blogs ON comment.id_blog = blogs.id_blog and comment.id_user=blogs.id_user
            INNER JOIN users ON blogs.id_user=users.id_user
              

            WHERE comment.id_blog=blogs.id_blog and comment.id_user=?
            ";
